...
#pragma endregion 
		...
		GameObject::Sptr lightParent = scene->CreateGameObject("Lights");
		{
			...
			lightParent->SetPostion(glm::vec3(-3, -4.0f, 20.0f));
			lightParent->AddChild(light);

			Light::Sptr lightComponent = light->Add<Light>();
			lightComponent->SetColor(glm::linearRand(glm::vec3(1.0f), glm::vec3(1.0f)));
			lightComponent->SetRadius(glm::linearRand(10.0f, 10.0f));
		...
		{
			camera->SetPostion({ 0, -7, 7 });
			camera->SetRotation(glm::vec3(50.f, 0.f, 0.f));
		}
		...
		GameObject::Sptr monkey1 = scene->CreateGameObject("Monkey 1");
		{
			...
			monkey1->SetRotation(glm::vec3(0, 0, 90));
		...
		{
			// Set position in the scene
			monkey2->SetPostion(glm::vec3(5.f, 0.0f, 1.0f));
			monkey2->SetRotation(glm::vec3(0, 0, 90));
		...
		GameObject::Sptr Brick1 = scene->CreateGameObject("Normal Mapped Object");
		{
			...
			Brick1->SetPostion(glm::vec3(-11, -4.0f, 1.0f));
			Brick1->SetScale(glm::vec3(100, 1.0, 11));
			Brick1->SetRotation(glm::vec3(0 , 0, 90));

			// Add a render component 
			RenderComponent::Sptr renderer = Brick1->Add<RenderComponent>();
		

			renderer->SetMesh(boxMesh);
			renderer->SetMaterial(normalmapMat);

			demoBase->AddChild(Brick1);
		}

		GameObject::Sptr Brick2 = scene->CreateGameObject("Normal Mapped Object");
		{
			// Set and rotation position in the scene 
			...
			boxMesh->AddParam(MeshBuilderParam::CreateCube(ZERO, ONE));
			boxMesh->GenerateMesh();
			Brick2->SetPostion(glm::vec3(5, 10.0f, 1.0f));
			Brick2->SetScale(glm::vec3(100, 1.0f, 11.0f));

			// Add a render component 
			RenderComponent::Sptr renderer = Brick2->Add<RenderComponent>();


			renderer->SetMesh(boxMesh);
			renderer->SetMaterial(normalmapMat);

			demoBase->AddChild(Brick2);


		}

		GameObject::Sptr Brick3 = scene->CreateGameObject("Normal Mapped Object");
		{
			// Set and rotation position in the scene 
			...
			boxMesh->AddParam(MeshBuilderParam::CreateCube(ZERO, ONE));
			boxMesh->GenerateMesh();
			Brick3->SetPostion(glm::vec3(11, -4.0f, 1.0f));
			Brick3->SetScale(glm::vec3(100, 1.0, 11));
			Brick3->SetRotation(glm::vec3(0, 0, 90));

			// Add a render component 
			RenderComponent::Sptr renderer = Brick3->Add<RenderComponent>();


			renderer->SetMesh(boxMesh);
			renderer->SetMaterial(normalmapMat);

			demoBase->AddChild(Brick3);
